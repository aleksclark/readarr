...
namespace NzbDrone.Core.Books
{
    ...
    public class AddBookService : IAddBookService
    {
        ...
        private Book AddSkyhookData(Book newBook)
        {
            ...
            {
                _logger.Error("Book with Foreign Id {0} was not found, it may have been removed from the metadata source.", newBook.ForeignBookId);

                throw new ValidationException(new List<ValidationFailure>
                                              {
                                                  new ValidationFailure("MetadataId", "A book with this ID was not found", newBook.ForeignBookId)
                                              });
            }
